...
def alpha_beta_pruning(board: Board, depth, alpha, beta, maximizingPlayer, nn_model, moves=None):
    ...
    if depth == 1:
        ...
        pos = np.zeros((len(moves), 773))
        i = 0
        for m in moves:
            copy_board = board.copy()
            copy_board.push(m)
            pos[i] = get_board_state(copy_board)
            i += 1

        res_evals = lmodel.predict(pos)

        if maximizingPlayer:
            res_val = max(res_evals)
        else:
            res_val = min(res_evals)

        return res_val, moves[np.where(res_evals == res_val)[0][0]]
...
if __name__ == '__main__':
    ...
    # todo check za last state, i u search i u rl
    # todo probaj da evaluiras sve odjednom na depth = 0
